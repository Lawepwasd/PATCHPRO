...
class MultiAttack(Attack):
    ...
    def __init__(self, attacks, verbose=False, bitmap = None):
    ...
    def forward(self, images, labels):
        ...
        if self.bitmap is not None:
            # save the bitmap
            bitmap_copy = self.bitmap.clone().detach().to(self.device)
        for i, attack in enumerate(self.attacks):
            

            if self.bitmap is not None:
                # updata the multi attack bitmap
                self.bitmap = bitmap_copy[fails]
                #  update the single attack bitmap         
                attack.bitmap = bitmap_copy[fails]
            ...
            # This modify end
            if self.bitmap is not None:
                self.bitmap = attack.bitmap.clone().to(self.device)
