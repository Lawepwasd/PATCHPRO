...
def test_repaired(args: Namespace) -> float:
    ...
    rear_label.load_state_dict(torch.load(Path(LABEL_MODEL_DIR, f'Cifar-{args.net}-feature-repair_number{args.repair_number}-rapair_radius{args.repair_radius}-label_repaired.pt')))
    ...
    if Path(RES_DIR, f'best_{args.net}_{args.repair_radius}_{args.repair_number}_adv_train.pth').exists():
        ...
        logging.info(f'--adv training')
        adv_train_net = adv_train(epoch=200)
