...
class CifarArgParser(exp.ExpArgParser):
    ...
    def __init__(self, log_path: Optional[str], *args, **kwargs):
        ...
        self.add_argument('--label-repaired', type=bool, default=False,
                            help='use label format or not')
...
def repair_cifar(args: Namespace)-> Tuple[int, float, bool, float]:
    ...
    if args.label_repaired:
        feature_repair_prop_list = CifarProp.all_feature_props(args.dom, bounds = feature_repair_feature_bounds, labels= repairset.labels, feature_shape = feature_shape,tasktype='feature_input_label')
    else:
        feature_repair_prop_list = CifarProp.all_feature_props(args.dom, bounds = feature_repair_feature_bounds, labels= repairset.labels, feature_shape = feature_shape,tasktype='feature_input')
    ...
    # the following it to get the bitmap of the feature from their inputs
    input_shape = trainset.inputs.shape[1:]
    repairlist = [(data[0],data[1]) for data in zip(originalset.inputs, originalset.labels)]
    if args.label_repaired:
        repair_prop_list = CifarProp.all_props(args.dom, DataList=repairlist, input_shape= input_shape,radius= args.repair_radius/255, tasktype='attack_input_label')
    else:
        repair_prop_list = CifarProp.all_props(args.dom, DataList=repairlist, input_shape= input_shape,radius= args.repair_radius/255, tasktype='attack_input')
    ...
    for o in range(args.divided_repair):
        ...
        if args.label_repaired:
            torch.save(repair_net.state_dict(), str(LABEL_REPAIR_MODEL_DIR / f'Cifar-{args.net}-{args.repair_location}-repair_number{args.repair_number}-rapair_radius{args.repair_radius}-label_repaired.pt'))
        else:
...
def test(lr:float = 0.005, net:str = 'CNN_small',repair_radius:float = 0.1, repair_number = 200, refine_top_k = 300,
         ...
         accuracy_loss:str = 'CE',label_repaired = False):
    test_defaults = {
        'net': net,
    ...
    if not label_repaired:
        parser = CifarArgParser(RES_DIR, description='CIFAR10 Correct by Construction')
    else:
        parser = CifarArgParser(LABEL_RES_DIR, description='CIFAR10 Label Correct by Construction')
   ...
   # for net in ['FNN_small', 'FNN_big', 'CNN_small']:
    ...
    # for net in ['vgg19']:
        ...
        # for patch_size in ['big']:
            ...
            accuracy_loss='CE',label_repaired = label_repaired)
